...
public class MetavanteRespFileHandler implements
   com.ffusion.ffs.bpw.interfaces.DBConsts,FFSConst,
   com.ffusion.ffs.bpw.interfaces.BPWResource, BPWScheduleHandler
{
   //private MetavanteRespFileHandlerImpl _respFile = null;
   private boolean _okToCall;

   public MetavanteRespFileHandler(){
	   // _respFile = new MetavanteRespFileHandlerImpl();
   }
...
//=====================================================================
   ...
   throws Exception
   {
	   String methodName = "MetavanteRespFileHandler.eventHandler";
       long start = System.currentTimeMillis();
       int uniqueIndex = PerfLoggerUtil.startPerfLoggingAndGetUniqueIndex(methodName,start);

      FFSDebug.log("MetavanteRespFileHandler.eventHandler: begin, eventSeq=" + eventSequence, PRINT_DEV);
      if ( eventSequence == 0 ) {         // FIRST sequence
          ...
          if (_okToCall){
               try {
  /*               Class classDefinition = Class.forName("com.ffusion.ffs.bpw.fulfill.metavante.MetavanteRespFileHandlerImpl");
  ...
  */      
            	    Map<String,Object> extra = new HashMap<String,Object>();   
          			...
          			BPWFulfillmentRespFileHandler fulfillmentRespFileHandler = fulfillmentHandlerProvider.getBPWFulfillmentRespFileHandler("com.ffusion.ffs.bpw.fulfill.metavante.MetavanteRespFileHandlerImpl");
           ...
           // _respFile.processResponseFiles(dbh);
      }
      else if ( eventSequence == 3 ) {    // Batch-Start sequence
      }
      else if ( eventSequence == 4 ) {    // Batch-End sequence
      }
      FFSDebug.log("MetavanteRespFileHandler.eventHandler: end", PRINT_DEV);
      PerfLoggerUtil.stopPerfLogging(methodName, start, uniqueIndex);
   }
...
//=====================================================================
   ...
   throws Exception
   {
	   String methodName = "MetavanteRespFileHandler.resubmitEventHandler";
       long start = System.currentTimeMillis();
       int uniqueIndex = PerfLoggerUtil.startPerfLoggingAndGetUniqueIndex(methodName,start);
      FFSDebug.log("=== MetavanteRespFileHandler.resubmitEventHandler: begin, eventSeq="
                                      + eventSequence
                                      + ",length="
                                      + evts._array.length
                                      + ",instructionType="
