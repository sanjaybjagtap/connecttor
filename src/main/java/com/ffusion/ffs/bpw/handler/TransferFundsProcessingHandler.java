...
/**
 * This calss passes all matured transfers to the bank backend for
 * funds processing. This handler will be triggered by ETFFUNDTRN
 * schedule.
 */
public class TransferFundsProcessingHandler extends TransferFundsBaseHandler implements BPWScheduleHandler
{
     ...
     *			Status=WILLPROCESSON and a predetermined ProcessingDate
     ...
     *			recovery.
     *		iii. 	Update the status of the transfers from WILLPROCESSON to
     *			INFUNDSPROCESSING
     *		iv.  	Call the backend to process the transfers
     * 		 v.  	If errors occur in any of the above steps, log this fact and
     *			return from this method
     */
    public void eventHandler( int eventSequence, EventInfoArray evts, FFSConnectionHolder dbh ) throws Exception
    {
	final String methodName = "TransferFundsProcessingHandler.eventHandler:";
     ...
     *			FundsProcessing=BPW_TRANSFER_FUNDS_PROCESSING_SUPPORTED,
     *			Status=WILLPROCESSON, LastProcessId = process id of
     ...
     *			transfers with the given FIId,
     *			FundsProcessing=BPW_TRANSFER_FUNDS_PROCESSING_SUPPORTED,
     *			Status=WILLPROCESSON and LastProcessId = process Id of schedule
     ...
     *			database as this will help recover transfers during a crash
     *			recovery.
     *		iv.  	Update the status of the transfers from WILLPROCESSON to
     *			INFUNDSPROCESSING
     ...
     *			database as this will help recover transfers during a crash
     *			recovery.
     *		ix. 	Update the status of the transfers from WILLPROCESSON to
     *			INFUNDSPROCESSING
    ...
    {
	final String methodName = "TransferFundsProcessingHandler.resubmitEventHandler:";
	long start = System.currentTimeMillis();
    int uniqueIndex = PerfLoggerUtil.startPerfLoggingAndGetUniqueIndex(methodName,start);
    ...
    {
    	String method = "TransferFundsProcessingHandler.initProcessTrans:";
    	long start = System.currentTimeMillis();
        ...
        _transferFundsProcessing.processScheduledTransferFunds(transferInfos, null);
        PerfLoggerUtil.stopPerfLogging(methodName, start, uniqueIndex);
    }
    ...
    // This is the method where we retrieve the transfers from the database
    // and then send  them to the backend for processing
    // @param dbh contains information about the database connection
    ...
    //		based on the FIId, FundsProcessing=BPW_TRANSFER_FUNDS_PROCESSING_SUPPORTED,
    //		Status=WILLPROCESSON and LastProcessId = process id of schedule
    //		- for regular processing, the transfers are looked up in the database based on
    //		the FIId, FundsProcessing=BPW_TRANSFER_FUNDS_PROCESSING_SUPPORTED,
    //		Status=WILLPROCESSON and ProcessingDate=today
    ...
    //		iii.	update the status of the transfers from WILLPROCESSON to INFUNDSPROCESSING
    //			note that this is done BEFORE the backend call and the backend may
    //			change the status again which will make that change take precedence
	...
	FFSConnectionHolder dbhdr = null;
	...
	try {
		...
		// is file-based, we need to look for transfers in the database
		...
		// based on FIId, FundsProcessing=2, Status=WILLPROCESSON,
		// and LastProcessId = process Id of schedule
		stmt1SQL.append( SQLConsts.GET_RESUBMIT_TRANSFER_FOR_NONFILEBASED_ETFFUNDTRN );
	    } else {
		...
		// FundsProcessing=2, Status=WILLPROCESSON and processDate=today
	    	 if( _processId != null && _processId.equals("0") ) {
	    		 stmt1SQL.append( SQLConsts.GET_TRANSFER_FOR_IMMEDIATE_ETFFUNDTRN );
	    	 } else {
	    		 stmt1SQL.append( SQLConsts.GET_TRANSFER_FOR_ETFFUNDTRN );
	    	 }
		
	    }
	    //QTS 586287, commenting order by
	    ...
	    stmt2 = con.prepareStatement( SQLConsts.UPDATE_TRANSFER_STATUS_BASEDON_SRVRTID_STATUS );

	    //stmt3 = con.prepareStatement( SQLConsts.UPDATE_TRANSFER_PROCESSNUMBER_AND_PROCESSID );
	    stmt3 = con.prepareStatement(SQLConsts.UPDATE_TRANSFER_PROCESSNUMBER_AND_PROCESSID_BASEDON_SRVRTID_STATUS);
	    ...
	    //Get a Dirty read to get Transfer Info. this is to avoid locking for Transfers mods and cancels when this schedule is running
	    dbhdr = new FFSConnectionHolder();
	    dbhdr.conn = DBUtil.getDirtyReadConnection();
		...
		while( rset.next() )
		 {
		    TransferInfo transferInfo = getTransferInfo( dbhdr, rset, extraMap, methodName );
		    EntChannelOps entChannelOps = ChannelStore.getChannelIdForTrn(transferInfo.getSubmittedBy(), transferInfo.getSrvrTId(), 
	    			DBConsts.BPW_XferInstruction, dbh);
		    
		    transferInfo.setChannelId(entChannelOps.getChannelIDMod());
		    
		    /**
		     * Added this block to avoid duplicate processing of immediate external transfer. When schedule is running, at the same time if user added immediate external transfer,
		     * Then schedule pickup both immediate and schedule transfer for processing. This will result into duplication of immediate external transfer.
		     */
		    if( _processId != null && _processId.equals("0") ) {
		    	//For Immediate Transfer do nothing.
		    } else {
		    	Hashtable extraInfo = transferInfo.getExtInfo();
		    	if(extraInfo != null) {
		    		Object object = extraInfo.get(DBConsts.IMMEDIATE_PROCESSING);
		    		if (object != null && object instanceof ValueInfo){
		    			ValueInfo valInfo = (ValueInfo) object;
			    		if(valInfo != null) {
			    			Object objValueInfo = valInfo.getValue();
			    			if(objValueInfo != null && objValueInfo instanceof String) {
					    		String isImmediateProcessing = (String) objValueInfo;
					    		if(isImmediateProcessing != null && isImmediateProcessing.trim().equalsIgnoreCase(DBConsts.TRUE)) {
					    			FFSDebug.log("Transfer with srvrTid = " + transferInfo.getSrvrTId() + " is for immediate processing. Skipping this transfer in schedule processing.", PRINT_DEV);
					    			continue;
					    		}
			    			}
			    		}
		...
		if( resultCount > 0 ) {
			...
			stmt3.setString( 4, WILLPROCESSON );

			stmt3.executeUpdate();
		    }
		    con.commit();

		    // update the status of the transfers from WILLPROCESSON to
		    // INFUNDSPROCESSING but do not commit this change
		    for( int i = 0; i < size; i++ ) {
			stmt2.clearParameters();
			stmt2.setString( 1, INFUNDSPROCESSING );
			stmt2.setString( 2, transferInfos[i].getSrvrTId() );
			stmt2.setString( 3, WILLPROCESSON );
		    ...
		    // call the backend to process the transfers
		    if( _processId != null && _processId.equals("0") ) {
		    	_transferFundsProcessing.processImmediateTransferFunds( transferInfos, tab );
		    	resultCount = 0;
		    } else {
		    	_transferFundsProcessing.processScheduledTransferFunds( transferInfos, tab );
		    }
		    
            if (_doAuditLogging)
		    {
                // audit log transfer objects after processing
                for (int i=0; i < size; i++)
                {
                    msg = BPWLocaleUtil.getLocalizableMessage(ACHConsts.PROCESSING_TRANSFER,
                                                          new Object[] {transferInfos[i].getSrvrTId()},
                                                          BPWLocaleUtil.TRANSFER_MESSAGE);
                    auditLog( dbh, transferInfos[i], AuditLogTranTypes.BPW_ETFFUNDTRN, msg, methodName );
                }
            }
        }
        }
		...
		for( int i = 0; i < size; i++ )
		{
		    ...
		    if( _doAuditLogging )
		    {
				ILocalizable auditMsg = BPWLocaleUtil.getLocalizableMessage(ACHConsts.PROCESSING_TRANSFER_FAILED,
																			...
																			BPWLocaleUtil.TRANSFER_MESSAGE );
				auditLogError( transferInfos[i], AuditLogTranTypes.BPW_ETFFUNDTRN, auditMsg, methodName );
		    }
		}
	    }
	    ...
	    if( _batchKey != null && (!possibleDuplicate)) {
		DBConnCache.unbind( _batchKey );
	    }

	    if ((dbhdr != null) && (dbhdr.conn!=null)) {
	        DBUtil.freeConnection(dbhdr.conn);
	    	                dbhdr = null;
	    }
