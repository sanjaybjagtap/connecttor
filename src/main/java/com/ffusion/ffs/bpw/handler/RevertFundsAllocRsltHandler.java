...
import com.ffusion.ffs.bpw.custimpl.interfaces.RevertFundsAllocResult;
...
public class RevertFundsAllocRsltHandler implements DBConsts, FFSConst
{
    private RevertFundsAllocResult _fundsRslt = null;
    private boolean _okToCall;
    private static int _batchNum = 1;
    private final String BATCHKEYPREFIX = "RevertFundsRslt_";

    public RevertFundsAllocRsltHandler()
    {

    	try{
    		BackendProvider backendProvider = getBackendProviderService();
    		_fundsRslt = backendProvider.getRevertFundsAllocResultInstance();

    	} catch(Throwable t){
    		FFSDebug.log("Unable to get RevertFundsAllocResult Instance" , PRINT_ERR);
    	}
    }
...
//=====================================================================
    ...
    throws Exception
    {
    	String methodName = "RevertFundsAllocRsltHandler.eventHandler";
        long start = System.currentTimeMillis();
        int uniqueIndex = PerfLoggerUtil.startPerfLoggingAndGetUniqueIndex(methodName,start);
        FFSDebug.log("=== RevertFundsAllocRsltHandler.eventHandler: begin, eventSeq="
        ...
        if ( eventSequence == 0 ) {         // FIRST sequence
            ...
            if (_okToCall) {
               ...
               _fundsRslt.ProcessRevertFundsAllocRslt(batchKey);
        ...
        FFSDebug.log("=== RevertFundsAllocRsltHandler.eventHandler: end", FFSConst.PRINT_DEV);
...
//=====================================================================
    ...
    throws Exception
    {
    	String methodName = "RevertFundsAllocRsltHandler.resubmitEventHandler";
        long start = System.currentTimeMillis();
        int uniqueIndex = PerfLoggerUtil.startPerfLoggingAndGetUniqueIndex(methodName,start);
        FFSDebug.log("==== RevertFundsAllocRsltHandler.resubmitEventHandler: begin, eventSeq="
                     + eventSequence, FFSConst.PRINT_DEV);
        if ( eventSequence == 0 ) {         // FIRST sequence
            _okToCall = false;
        }
        ...
        FFSDebug.log("==== RevertFundsAllocRsltHandler.resubmitEventHandler: end", FFSConst.PRINT_DEV);
        PerfLoggerUtil.stopPerfLogging(methodName, start, uniqueIndex);
    }
