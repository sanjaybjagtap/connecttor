...
import com.ffusion.ffs.bpw.custimpl.interfaces.SamplePmtResult;
...
public class SamplePmtRsltHandler implements
   com.ffusion.ffs.bpw.interfaces.DBConsts,FFSConst,
   com.ffusion.ffs.bpw.interfaces.BPWResource, BPWScheduleHandler
{
   private SamplePmtResult _pmtRslt = null;
   private boolean _okToCall;

   public SamplePmtRsltHandler()
   {
	   
	    try{
    		BackendProvider backendProvider = getBackendProviderService();
    		_pmtRslt = backendProvider.getSamplePmtResultInstance();

    	} catch(Throwable t){
    		FFSDebug.log("Unable to get SamplePmtResult Instance" , PRINT_ERR);
    	}
   }
...
//=====================================================================
   ...
   throws Exception
   {
	   String methodName = "SamplePmtRsltHandler.eventHandler";
       long start = System.currentTimeMillis();
       int uniqueIndex = PerfLoggerUtil.startPerfLoggingAndGetUniqueIndex(methodName,start);
      FFSDebug.log("=== SamplePmtRsltHandler.eventHandler: begin, eventSeq=" + eventSequence, PRINT_DEV);
      if ( eventSequence == 0 ) {         // FIRST sequence
         ...
         if (_okToCall) {
            ...
            // Dummy implementation to update payee status
            updatePayeesStatus(dbh);
      ...
      FFSDebug.log("=== SamplePmtRsltHandler.eventHandler: end", PRINT_DEV);
...
//=====================================================================
   ...
   throws Exception
   {
	   String methodName = "SamplePmtRsltHandler.resubmitEventHandler";
       long start = System.currentTimeMillis();
       int uniqueIndex = PerfLoggerUtil.startPerfLoggingAndGetUniqueIndex(methodName,start);
      FFSDebug.log("==== SamplePmtRsltHandler.resubmitEventHandler: begin, eventSeq=" + eventSequence);
      if ( eventSequence == 0 ) {         // FIRST sequence
          _okToCall = false;
      }
      else if ( eventSequence == 1 ) {    // NORMAL sequence
   ...
   /**
	...
	/**
	 * Update payee status to active, assume that customer has validated payee.	 * @param dbh FFSConnectionHolder Object
	 */
	private void updatePayeesStatus(FFSConnectionHolder dbh) {
		try {
			PayeeInfo[] payeeList = Payee.findPayeesByStatus(DBConsts.INPROCESS, dbh);
			int payeeListSize = payeeList.length;
			
			for (int i=0; i<payeeListSize; i++) {
				Payee.updateStatus(payeeList[i].PayeeID, DBConsts.ACTIVE, dbh);
			}
			dbh.conn.commit();
		} catch (Exception ex) {
			FFSDebug.log("Unable to update payee status");
		}
	}
