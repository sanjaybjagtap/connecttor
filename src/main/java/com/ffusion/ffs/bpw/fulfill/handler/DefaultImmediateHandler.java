...
public class DefaultImmediateHandler implements IImmediateBillPayHandler
{
	public PmtInfo[] processBillPayment(PmtInfo[] payments, String batchKey, HashMap extraInfo) throws Exception
	{
		final String currentMethod = "DefaultImmediateHandler.processBillPayment";
		long start = System.currentTimeMillis();
        int uniqueIndex = PerfLoggerUtil.startPerfLoggingAndGetUniqueIndex(currentMethod,start);
		...
		if(payments != null && payments.length > 0)
		{
			...
			catch(Exception e)
			{
				...
				PerfLoggerUtil.stopPerfLogging(currentMethod, start, uniqueIndex);
	...
	{
		String method = "DefaultImmediateHandler.getIntraTransferInfoFromPmtInfo";
        long start = System.currentTimeMillis();
