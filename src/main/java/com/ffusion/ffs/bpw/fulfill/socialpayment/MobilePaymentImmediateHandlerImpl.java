...
public class MobilePaymentImmediateHandlerImpl implements IImmediateBillPayHandler
{

	@Override
	public PmtInfo[] processBillPayment(PmtInfo[] payments, String batchKey, HashMap extraInfo) throws Exception {

		final String currentMethod = "MobilePaymentImmediateHandlerImpl.processBillPayment";
		...
		if (payments != null && payments.length > 0) {
			...
			try {
				ToBackend intraBackendHandler = getMobilePaymentBackendHandler();
				...
				FFSDebug.log(currentMethod + " exception during MobilePaymentImmediateHandlerImpl.processBillPayment: " + e.toString(), FFSConst.PRINT_DEV);
				throw e;
			}
	 ...
	 * @throws Exception
	 */
	private ToBackend getMobilePaymentBackendHandler() throws Exception
	{
